...
const AvailabilityModal = ({ isOpen, onClose, onAvailable, text }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative transform animate-fade-in-up">
                <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-800 transition-colors">
                    <CloseIcon className="w-6 h-6" />
                </button>
                <h2 className="text-3xl font-bold text-center mb-2">{text.availabilityModalTitle}</h2>
                <p className="text-gray-500 text-center mb-8">{text.availabilityModalDesc}</p>
                <form onSubmit={handleCheck}>
                    <div>
                        <label htmlFor="postalCode" className="block text-sm font-medium text-gray-700">{text.postalCodeLabel}</label>
                        <input type="text" id="postalCode" value={postalCode} onChange={(e) => setPostalCode(e.target.value.toUpperCase())} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="A1A 1A1" />
                        {error && <p className="text-red-500 text-sm mt-1">{error}</p>}
                    </div>
                    <div className="mt-8">
                        <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105 disabled:bg-gray-400 disabled:scale-100">
                            {isLoading ? text.checkingBtn : text.checkAvailabilityBtn}
                        </button>
                    </div>
                </form>
            </div>
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative transform animate-fade-in-up">
                ...
                <h2 className="text-3xl font-bold text-center mb-2">{text.signUpModalTitle}</h2>
                <p className="text-gray-500 text-center mb-8">{text.signUpModalDesc}</p>
                <form>
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="fullName" className="block text-sm font-medium text-gray-700">{text.fullNameLabel}</label>
                            <input type="text" id="fullName" className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-gray-700">{text.emailLabel}</label>
                            <input type="email" id="email" className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                    </div>
                    <div className="mt-8">
                        <button type="submit" className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105">
                            {text.completeSignUpBtn}
                        </button>
                    </div>
                </form>
            </div>
               ...
               <div className="relative">
                    ...
                    {isLangDropdownOpen && (
                        <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-xl py-1 text-gray-800 animate-fade-in-down">
                            <a href="#" onClick={() => { setLanguage('en'); setIsLangDropdownOpen(false); }} className="block px-4 py-2 text-sm hover:bg-gray-100">English</a>
                            <a href="#" onClick={() => { setLanguage('fr'); setIsLangDropdownOpen(false); }} className="block px-4 py-2 text-sm hover:bg-gray-100">Français</a>
                            <a href="#" onClick={() => { setLanguage('sp'); setIsLangDropdownOpen(false); }} className="block px-4 py-2 text-sm hover:bg-gray-100">Español</a>
                        </div>
                    )}
               </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            <div>
              ...
              <ul className="space-y-3">
                <li><a href="#home" className="text-gray-400 hover:text-white">Home Internet</a></li>
                <li><a href="#business" className="text-gray-400 hover:text-white">Business Internet</a></li>
                <li><a href="#esim" className="text-gray-400 hover:text-white">Global eSIM</a></li>
              </ul>
            </div>
            <div>
