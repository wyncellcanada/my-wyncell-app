...
// This is the component for each country/region card
function PackageCard({ pkg }) {
  const { text, language } = useAppContext(); // Get language from context
  const firstPackage = pkg.operators[0]?.packages[0];
  if (!firstPackage) return null;

  // --- THIS IS THE FIX ---
  // 1. Determine the currency, defaulting to USD if not provided.
  const currency = pkg.operators[0]?.currency || 'USD';

  // 2. Use the Internationalization API to format the price correctly.
  const formattedPrice = new Intl.NumberFormat(language, {
    style: 'currency',
    currency: currency,
    minimumFractionDigits: 2, // Always show two decimal places, e.g., $4.50
    maximumFractionDigits: 2,
  }).format(firstPackage.price);
  // --- END FIX ---
  ...
  return (
    <Link href={`/esims/${pkg.slug}`} className="group block rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div className="p-4 bg-white">
        ...
        {/* Use the new, correctly formatted price */}
...
// This is the main component for the entire /esims page
export default function EsimsPage() {
    ...
    async function fetchAllData() {
      ...
      try {
        const [localResponse, globalResponse] = await Promise.all([
          fetch(`/api/packages?type=local`),
          fetch(`/api/packages?type=global`)
        ]);
  ...
  const filteredPackages = useMemo(() => {
    ...
    const validPackages = packagesToShow.filter(pkg => pkg.operators && pkg.operators[0]?.packages[0]);
    if (!searchTerm) {
